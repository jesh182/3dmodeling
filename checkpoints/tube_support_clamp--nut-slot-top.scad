...
// ── Nut slot — from socket top down to nut position ───────────────────────────
// Tube and nut insert from above; slot gives clearance from top to nut centre.
module nut_slot() {
    slot_bottom = SADDLE_Z + NUT_FROM_FLOOR - NUT_SLOT_W / 2;
    slot_top    = SADDLE_Z + SOCKET_DEPTH + 0.02;
    translate([CX - NUT_SLOT_W / 2, CY - NUT_SLOT_T / 2, slot_bottom])
        cube([NUT_SLOT_W, SOCKET_OD, slot_top - slot_bottom]);
}
